...
def _should_use_virtualenv() -> bool:
    ...
    return virtualenv is not None and not any(
        lib.vendor.packaging.requirements.Requirement(d[1]).name == 'virtualenv'
...
def _create_isolated_env_venv(path: str) -> tuple[str, str]:
    ...
    import lib.vendor.packaging.version
    ...
    current_pip_version = lib.vendor.packaging.version.Version(pip_distribution.version)
    ...
    if current_pip_version < lib.vendor.packaging.version.Version(minimum_pip_version):
