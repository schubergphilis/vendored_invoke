...
from lib.vendor.humanfriendly import coerce_boolean
from lib.vendor.humanfriendly.compat import coerce_string, is_string, on_windows
from lib.vendor.humanfriendly.terminal import ANSI_COLOR_CODES, ansi_wrap, enable_ansi_support, terminal_supports_colors
from lib.vendor.humanfriendly.text import format, split
...
def install(level=None, **kw):
    ...
    if not (handler and not reconfigure):
        ...
        if syslog_enabled not in (None, False):
            from lib.vendor.coloredlogs.syslog import enable_system_logging
