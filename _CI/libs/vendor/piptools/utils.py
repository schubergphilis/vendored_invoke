...
from libs.vendor.click.utils import LazyFile
...
from libs.vendor.piptools._compat import PIP_VERSION
from libs.vendor.piptools.subprocess_utils import run_python_snippet
...
def key_from_req(req: InstallRequirement | Distribution | Requirement) -> str:
    ...
    if hasattr(req, "key"):
        ...
        # from libs.vendor.packaging. such as install requirements from requirements.txt
        key = req.name
...
def get_compile_command(click_ctx: click.Context) -> str:
    ...
    """
    from libs.vendor.piptools.scripts.compile import cli
